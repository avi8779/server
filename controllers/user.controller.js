...
export const markLectureAsWatched = async (req, res) => {
  const { userId, lectureId } = req.body;

  try {
    // Logic to mark the lecture as watched
    ...
    // Mark the lecture as watched (This part depends on your schema)
    user.watchedLectures.push(lectureId);
    await user.save();

    return res.status(200).json({ message: "Lecture marked as watched" });
  } catch (error) {
